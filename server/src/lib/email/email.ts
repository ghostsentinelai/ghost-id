...
export const sendEmail = async (email: string, subject: string, html: string) => {
  ...
  try {
    const response = await resend.emails.send({
      from: "GHOST ID <automail@email.ghost-id.com>",
      to: email,
      subject,
      html,
    });
...
const OTP_SUBJECTS: Record<OtpEmailType, string> = {
  "sign-in": "Your GHOST ID Sign-In Code",
...
export const sendInvitationEmail = async (
  ...
  await sendEmail(email, "You're Invited to Join an Organization on GHOST ID", html);
};
...
export const sendLimitExceededEmail = async (
  ...
  const upgradeLink = "https://app.ghost-id.io/settings/organization/subscription";
...
Welcome to GHOST ID! Thanks for signing up.

I'm excited to have you on board. GHOST ID is fully self-funded and we're fully committed to making an analytics platform that only serves the interests of our users.
...
Bill`;
  ...
  try {
    await resend.emails.send({
      from: "Bill from GHOST ID <bill@email.ghost-id.com>",
      replyTo: "hello@ghost-id.com",
      to: email,
      subject: "Welcome to GHOST ID!",
      text,
    });
  } catch (error) {
