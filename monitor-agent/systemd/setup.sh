...
# Setup script for GHOST ID Monitor Agent with systemd
...
# Configuration
INSTALL_DIR="/opt/ghost-id-monitor-agent"
SERVICE_USER="ghost-id"
SERVICE_NAME="ghost-id-monitor-agent"
NODE_VERSION="20"
...
# GHOST ID Monitor Agent Configuration
NODE_ENV=production
PORT=3000
HOST=0.0.0.0
...
# Main Server
MAIN_SERVER_URL=https://your-ghost-id-server.com
...
cp ghost-id-monitor-agent.service /etc/systemd/system/
...
cat > /etc/logrotate.d/ghost-id-monitor-agent << EOF
...
if command -v ufw &> /dev/null; then
    ...
    ufw allow 3000/tcp comment "GHOST ID Monitor Agent"
fi
