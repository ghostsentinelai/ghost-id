...
1. Navigate to **Settings → Account** in your GHOST ID dashboard
...
<Tab value="Pageview">
```bash
curl -X POST 'https://app.ghost-id.io/api/track' \
...
const trackEvent = async (eventData) => {
  const response = await fetch('https://app.ghost-id.io/api/track', {
    method: 'POST',
    headers: {
...
def track_event(event_data):
    response = requests.post(
        'https://app.ghost-id.io/api/track',
        headers={
            'Content-Type': 'application/json',
...
public class GHOST IDTracker {
    private static final String API_URL = "https://app.ghost-id.io/api/track";
    ...
    public static void main(String[] args) throws Exception {
        GHOST IDTracker tracker = new GHOST IDTracker();
...
class GHOST IDTracker {
    private $apiUrl = 'https://app.ghost-id.io/api/track';
    private $apiKey;
    private $siteId;
...
class GHOST IDTracker
  API_URL = 'https://app.ghost-id.io/api/track'
...
public class GHOST IDTracker
{
    ...
    private const string ApiUrl = "https://app.ghost-id.io/api/track";

    public GHOST IDTracker()
    {
    ...
    {
        var tracker = new GHOST IDTracker();
...
impl GHOST IDTracker {
    fn new() -> Self {
        Self {
            ...
            api_url: "https://app.ghost-id.io/api/track".to_string(),
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let tracker = GHOST IDTracker::new();
...
class GHOST IDTracker {
    static let shared = GHOST IDTracker()

    private let apiUrl = "https://app.ghost-id.io/api/track"
    ...
    func trackEvent(_ eventData: [String: Any]) async throws {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(domain: "GHOST IDTracker", code: 1, userInfo: [NSLocalizedDescriptionKey: "Tracking failed"])
        }
    }
...
Task {
    // Track a pageview
    try await GHOST IDTracker.shared.trackEvent([
        "type": "pageview",
        "pathname": "/api/users",
        "hostname": "api.example.com",
        "page_title": "User API Endpoint"
...
class GHOST IDTracker(private val context: Context) {
    companion object {
        private const val API_URL = "https://app.ghost-id.io/api/track"
...
class MainActivity : AppCompatActivity() {
    private val tracker = GHOST IDTracker(this)
...
class GHOST IDTracker {
  static final GHOST IDTracker _instance = GHOST IDTracker._internal();
  factory GHOST IDTracker() => _instance;
  GHOST IDTracker._internal();

  static const String _apiUrl = 'https://app.ghost-id.io/api/track';
...
<Callout type="info">
**Browser Tracking**: For standard browser-based tracking, use the [GHOST ID Web SDK](/docs/sdks/web) instead. The HTTP API is designed for server-side applications, mobile apps, and custom integrations.
</Callout>
