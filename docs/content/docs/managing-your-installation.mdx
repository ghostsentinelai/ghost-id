...
description: Managing your GHOST ID installation
---
...
GHOST ID is updated frequently with new features and bug fixes. You can update your installation by running the following commands:
...
<Callout>
If you are using your self-hosted GHOST ID without anyone else, you can disable new user signups by setting `DISABLE_SIGNUP=true` in the `.env` file at the root of the repository.
</Callout>
...
To help us improve GHOST ID, self-hosted instances automatically send anonymous usage statistics to our cloud service. This includes:

- Number of rows in each ClickHouse table
- Total database size
- Your GHOST ID version
