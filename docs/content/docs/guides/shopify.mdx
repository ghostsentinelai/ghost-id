---
title: "Shopify"
description: "Integrate GHOST ID with your Shopify store to capture events, track conversions, and analyze user behavior."
---
...
GHOST ID can be integrated with your Shopify store to capture events, track conversions, and analyze user behavior.

## How to Add GHOST ID to Shopify
...
To get the most out of GHOST ID with Shopify, you should track key e-commerce events. This typically involves adding small Liquid and JavaScript snippets to your theme files.
...
<script type="text/javascript">
  if (typeof window.ghost-id !== 'undefined' && {{ product | json }}) {
    const ghost-idProduct = {{ product | json }};
    const ghost-idCurrentVariant = {{ product.selected_or_first_available_variant | json }};

    window.ghost-id.event('view_item', {
      item_id: ghost-idCurrentVariant.sku || ghost-idCurrentVariant.id,
      item_name: ghost-idProduct.title,
      item_variant: ghost-idCurrentVariant.title === 'Default Title' ? null : ghost-idCurrentVariant.title,
      price: parseFloat(ghost-idCurrentVariant.price) / 100, // Price is in cents
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const trackAddToCart = function() {
    if (typeof window.ghost-id !== 'undefined' && {{ product | json }}) {
      ...
      window.ghost-id.event('add_to_cart', {
        item_id: ghost-idCurrentVariant.sku || ghost-idCurrentVariant.id,
        item_name: ghost-idProduct.title,
        item_variant: ghost-idCurrentVariant.title === 'Default Title' ? null : ghost-idCurrentVariant.title,
        price: parseFloat(ghost-idCurrentVariant.price) / 100,
        currency: {{ cart.currency.iso_code | json }},
...
Shopify's checkout process is on a separate domain (`checkout.shopify.com`), and direct script injection into the checkout pages (beyond the "Order status" page) is limited for most plans. However, GHOST ID's script, if loaded on your main store, should persist session information.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (checkoutButton && typeof window.ghost-id !== 'undefined' && {{ cart | json }}) {
    checkoutButton.addEventListener('click', function() {
      const ghost-idCart = {{ cart | json }};
      let items = [];
      ghost-idCart.items.forEach(function(item) {
        items.push({
      ...
      window.ghost-id.event('begin_checkout', {
        currency: ghost-idCart.currency,
        value: parseFloat(ghost-idCart.total_price) / 100,
        items: items
      });
    });
  }
...
<script type="text/javascript">
  if (typeof window.ghost-id !== 'undefined' && {{ checkout | json }}) {
    const ghost-idCheckout = {{ checkout | json }};
    let items = [];
    ghost-idCheckout.line_items.forEach(function(item) {
      items.push({
        item_id: item.sku || item.variant_id,
    ...
    window.ghost-id.event('purchase', {
      transaction_id: ghost-idCheckout.order_number,
      value: parseFloat(ghost-idCheckout.total_price),
      tax: parseFloat(ghost-idCheckout.tax_price),
      shipping: parseFloat(ghost-idCheckout.shipping_price),
      currency: ghost-idCheckout.currency,
      items: items
    });
  }
</script>
...
*   **Currency Conversion**: Prices in Shopify Liquid objects are often in cents. Ensure you divide by 100 to get the decimal value for GHOST ID.
...
*   **Shopify Customer Events**: For more robust server-side or client-side event tracking, especially for `add_to_cart` and `begin_checkout` without relying on DOM manipulation, explore [Shopify Customer Events](https://shopify.dev/docs/api/customer-events). This allows you to subscribe to events like "product added to cart" or "checkout started" and send data to GHOST ID via a custom pixel or by triggering client-side JavaScript. This is a more advanced but often more reliable method.
*   **Testing**: Thoroughly test each event in your GHOST ID dashboard after implementation. Use your browser's developer console to check for JavaScript errors.