---
title: Astro
description: Integrate GHOST ID Analytics with your Astro application for content-rich websites
---
...
This guide explains how to integrate GHOST ID with your Astro application for analytics tracking. Astro is a web framework for building content-rich websites, delivering minimal JavaScript by default.

## How to Add GHOST ID to Your Astro Site

The most common way to add a global script like GHOST ID's to an Astro site is by including it in a common layout component that is used across your pages, or directly within the `<head>` of your pages if you don't use a shared layout.
...
Open your layout file and add the GHOST ID script tag within the `<head>` section.
...
const ghost-idSiteId = "YOUR_SITE_ID"; // Replace with your actual Site ID
---
<!doctype html>
<html lang="en">
	<head>
...
After building and deploying your Astro site, open your GHOST ID dashboard to check if data is being received. You can also inspect your browser's network tab to confirm that `script.js` is loaded on your pages.
</Step>
</Steps>

<Callout type="info">
  **Page View Tracking in Astro**:
  - **MPA (Multi-Page Application) Mode**: By default, Astro builds multi-page applications. When a user navigates between different pages, a full page load occurs, and GHOST ID's script will naturally track this as a new page view.
  - **View Transitions**: If you are using Astro's [View Transitions API](https://docs.astro.build/en/guides/view-transitions/) for client-side navigation effects, GHOST ID's script should generally still detect these as page views if the URL changes. However, if you notice issues, you might need to manually trigger page views. You can listen to Astro's `astro:after-swap` event:
    ```javascript
...
<script>
  document.getElementById('my-astro-button')?.addEventListener('click', () => {
    if (window["ghost-id"]) {
      window["ghost-id"].event('astro_button_click', { source: 'MyButton.astro' });
    }
  });
</script>
```
...
export default function MyReactIsland() {
  const handleClick = () => {
    if (window["ghost-id"]) {
      window["ghost-id"].event('react_island_click', { island: 'MyReactIsland' });
    }
  };
