---
title: Docusaurus
description: Integrate GHOST ID Analytics with your Docusaurus v2+ website
---
...
This guide explains how to integrate GHOST ID with your Docusaurus v2+ website for analytics tracking. Docusaurus is a static site generator that builds a single-page application with React.

## How to Add GHOST ID to Your Docusaurus Site

Docusaurus provides a configuration option to add scripts or tags to the `<head>` of every page, which is the recommended way to include GHOST ID.

<Steps>
<Step>
### Retrieve Your Tracking Script Details

Navigate to your GHOST ID dashboard to obtain your **Site ID**. The script source will be `https://app.ghost-id.io/api/script.js`.
</Step>
...
You can add the GHOST ID script using the `scripts` array or the `headTags` array within your site configuration. Using `scripts` is often cleaner for simple script additions.
...
export default function MyCustomButton({ children, eventName, eventData }) {
  const handleClick = () => {
    if (typeof window !== 'undefined' && window.ghost-id) {
      window.ghost-id.event(eventName || 'docusaurus_custom_click', eventData || {});
    }
  };
...
```javascript filename="src/client-modules/ghost-id-events.js"
...
export function onRouteUpdate({location, previousLocation}) {
  ...
  if (typeof window !== 'undefined' && window.ghost-id && location.pathname !== previousLocation?.pathname) {
    // GHOST ID should auto-track pageviews, but if needed:
  ...
  document.querySelectorAll('a[target="_blank"]').forEach(link => {
    link.addEventListener('click', () => {
      if (window.ghost-id) {
        window.ghost-id.event('external_link_click', { href: link.href });
      }
    });
  });
}
...
Always ensure `window.ghost-id` is available before calling its methods in client-side scripts.