---
title: WordPress
description: Integrate GHOST ID Analytics with your WordPress site
---
...
You can integrate GHOST ID into your WordPress site to track user behavior and gather analytics. There are several ways to do this depending on whether you're using a hosted WordPress.com site or a self-hosted WordPress.org installation.
...
3. Replace `YOUR_SITE_ID` with your actual Site ID from your GHOST ID dashboard
...
Visit your site in a new browser tab, then check your GHOST ID dashboard to confirm data is being received.
</Step>
</Steps>
...
```php
function add_ghost-id_analytics() {
    ?>
    <script>
    (function() {
      var s = document.createElement('script');
...
add_action('wp_head', 'add_ghost-id_analytics');
```
...
3. Create a new folder called `ghost-id-analytics`
4. Inside that folder, create a file called `ghost-id-analytics.php` with this content:

```php
<?php
/**
 * Plugin Name: GHOST ID Analytics
 * Description: Adds GHOST ID Analytics tracking to your WordPress site
 * Version: 1.0
 */

function ghost-id_add_tracking_script() {
    ?>
    <script>
    (function() {
      var s = document.createElement('script');
...
add_action('wp_head', 'ghost-id_add_tracking_script');
```

5. Replace `YOUR_SITE_ID` with your actual Site ID
6. Go to **Plugins** in WordPress and activate "GHOST ID Analytics"
</Tab>
</Tabs>
...
3. Search for `ghost-id` - you should see the script tag
...
Many WordPress caching plugins aggressively cache pages. After adding GHOST ID:
...
2. **Exclude the script from optimization:**
   Some optimization plugins try to defer, combine, or minify scripts. If GHOST ID isn't working, check these plugins:
   - **Autoptimize**: Add `app.ghost-id.io` to the exclusion list
...
Security plugins like Wordfence or Sucuri may block external scripts. Add `app.ghost-id.io` to your allowlist if tracking isn't working.
...
### Self-hosted GHOST ID

If you're self-hosting GHOST ID, replace `https://app.ghost-id.io` with your own domain:

```html
<script>
(function() {
  var s = document.createElement('script');
  s.src = 'https://your-ghost-id-domain.com/api/script.js';
  s.defer = true;
  s.setAttribute('data-site-id', 'YOUR_SITE_ID');
  document.head.appendChild(s);
})();
