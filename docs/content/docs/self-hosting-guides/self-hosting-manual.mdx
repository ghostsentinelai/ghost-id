...
description: Manual setup guide for GHOST ID using Docker Compose without the setup script
---
...
This guide is for users who want to manually set up GHOST ID using Docker Compose without the setup script. This is useful if you already have a reverse proxy setup (like Nginx, Traefik, or Coolify), or if you want more control over the configuration.
...
```bash
git clone https://github.com/ghost-id-io/ghost-id.git
cd ghost-id
```
</Step>
...
services:
  backend:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.ghost-id-api.rule=Host(`your.domain.com`) && PathPrefix(`/api`)"
      - "traefik.http.routers.ghost-id-api.tls.certresolver=letsencrypt"
      - "traefik.http.services.ghost-id-api.loadbalancer.server.port=3001"
  
  client:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.ghost-id-client.rule=Host(`your.domain.com`)"
      - "traefik.http.routers.ghost-id-client.tls.certresolver=letsencrypt"
      - "traefik.http.services.ghost-id-client.loadbalancer.server.port=3002"
```
</Tab>
</Tabs>
</Step>
...
GHOST ID consists of these services:
...
services:
  ...
  backend:
    image: ghcr.io/ghost-id-io/ghost-id-backend:${IMAGE_TAG:-latest}
    ...
    labels:
      ...
      - traefik.http.routers.ghost-id-backend.entrypoints=https
      - traefik.http.routers.ghost-id-backend.tls=true
      - traefik.http.routers.ghost-id-backend.rule=Host(`${DOMAIN_NAME}`) && (Path(`/api`) || PathPrefix(`/api/`))
      - traefik.http.services.ghost-id-backend.loadbalancer.server.port=3001
      - traefik.http.routers.ghost-id-backend.priority=100
      - traefik.http.middlewares.ghost-id-forward-headers.headers.customrequestheaders.X-Forwarded-Proto=https
      - traefik.http.middlewares.ghost-id-forward-headers.headers.customrequestheaders.X-Forwarded-Host=${DOMAIN_NAME}
      - traefik.http.routers.ghost-id-backend.middlewares=ghost-id-forward-headers

  client:
    image: ghcr.io/ghost-id-io/ghost-id-client:${IMAGE_TAG:-latest}
    ...
    labels:
      - traefik.enable=true
      - traefik.docker.network=coolify
      - traefik.http.routers.ghost-id-client.entrypoints=https
      - traefik.http.routers.ghost-id-client.tls=true
      - traefik.http.routers.ghost-id-client.rule=Host(`${DOMAIN_NAME}`)
      - traefik.http.services.ghost-id-client.loadbalancer.server.port=3002
      - traefik.http.routers.ghost-id-client.priority=10
...
To deploy GHOST ID on Coolify, you need to:
...
- `ghost-id_clickhouse-data`: Analytics data
- `ghost-id_postgres-data`: User accounts and site configurations
...
These guides provide step-by-step instructions for integrating GHOST ID with your existing infrastructure.
