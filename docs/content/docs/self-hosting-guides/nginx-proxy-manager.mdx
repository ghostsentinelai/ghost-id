...
description: Set up GHOST ID with Nginx Proxy Manager without exposing ports directly to the host
---
...
This guide shows you how to set up GHOST ID with Nginx Proxy Manager without exposing ports directly to the host.
...
<Callout type="info">
This setup assumes you have NPM running and are capable of creating SSL certificates. You can also use any other reverse proxy as long as you can put it on the same Docker network as GHOST ID.
</Callout>
...
Create a new directory for your GHOST ID installation and create a `docker-compose.yml` file:

```yaml
services:
  ghost-id_clickhouse:
    image: clickhouse/clickhouse-server:25.4.2
    container_name: ghost-id_clickhouse
    volumes:
  ...
  ghost-id_postgres:
    image: postgres:17.4
    container_name: ghost-id_postgres
    environment:
  ...
  ghost-id_backend:
    ...
    container_name: ghost-id_backend
    environment:
      - NODE_ENV=production
      - CLICKHOUSE_HOST=http://ghost-id_clickhouse:8123
      - CLICKHOUSE_DB=${CLICKHOUSE_DB:-analytics}
      - CLICKHOUSE_PASSWORD=${CLICKHOUSE_PASSWORD:-frog}
      - POSTGRES_HOST=ghost-id_postgres
      - POSTGRES_PORT=5432
      - POSTGRES_DB=${POSTGRES_DB:-analytics}
      - POSTGRES_USER=${POSTGRES_USER:-frog}
      - POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-frog}
  ...
  ghost-id_client:
    ...
    container_name: ghost-id_client
    environment:
      - NODE_ENV=production
...
1. **Create Proxy Host**:
   ...
   - Scheme: `http`
   - Forward Hostname/IP: `ghost-id_client`
   - Forward Port: `3002`
...
2. **Add Custom Location**:
   - Location: `/api`
   - Scheme: `http`
   - Forward Hostname/IP: `ghost-id_backend`
   - Forward Port: `3001`

3. **Advanced Configuration** (in the custom location):
   ```nginx
   location /api/ {
       proxy_pass http://ghost-id_backend:3001;
...
ingress:
  - hostname: your.domain.com
    path: /api
    service: http://ghost-id_backend:3001
  - hostname: your.domain.com
    service: http://ghost-id_client:3002
...
- Verify both NPM and GHOST ID are on the same network
...
**502 Bad Gateway**:
- Verify GHOST ID services are healthy: `docker compose ps`
...
# Specific service
docker compose logs -f ghost-id_backend
