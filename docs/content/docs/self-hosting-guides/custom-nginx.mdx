...
description: Set up GHOST ID with your own Nginx installation instead of using the built-in Caddy server
---
...
This guide shows you how to set up GHOST ID with your own Nginx installation instead of using the built-in Caddy server.
...
- GHOST ID running with `--no-webserver` flag

## Setup Steps

<Steps>
<Step>
### Run GHOST ID without Caddy

First, set up GHOST ID to expose ports without the built-in webserver:
...
If you need to use custom ports for GHOST ID:
...
**502 Bad Gateway**: Check if GHOST ID services are running:
```bash
docker compose ps
docker compose logs
```
