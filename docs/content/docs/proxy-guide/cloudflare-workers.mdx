...
description: Use Cloudflare Workers to proxy GHOST ID tracking at the edge
---
...
Cloudflare Workers provide a serverless way to proxy GHOST ID tracking at the edge, close to your users worldwide. This delivers excellent performance with minimal configuration.
...
2. Name it `ghost-id-proxy` (or your preferred name)
...
```javascript
// Cloudflare Worker for GHOST ID Analytics Proxy

const RYBBIT_HOST = 'https://app.ghost-id.io';
...
async function handleRequest(request) {
  ...
  try {
    return await proxyToGHOST ID(request, url);
  } catch (error) {
...
async function proxyToGHOST ID(request, url) {
  // Map paths: /analytics/* → /api/*
  let ghost-idPath = url.pathname.replace('/analytics/', '/api/');

  // Build GHOST ID URL
  const ghost-idUrl = new URL(ghost-idPath, RYBBIT_HOST);
  ghost-idUrl.search = url.search; // Preserve query params
  ...
  // Build proxy request
  const proxyRequest = new Request(ghost-idUrl, {
...
```javascript
// Minimal Cloudflare Worker for GHOST ID
...
async function handleRequest(request) {
  ...
  if (url.pathname === '/analytics/script.js' || url.pathname === '/analytics/track') {
    const ghost-idPath = url.pathname.replace('/analytics/', '/api/');
    const ghost-idUrl = `${RYBBIT_HOST}${ghost-idPath}`;
...
async function handleRequest(request, env) {
  const RYBBIT_HOST = env.RYBBIT_HOST || 'https://app.ghost-id.io';
  const url = new URL(request.url);
  ...
  return fetch(ghost-idUrl.toString(), {
    method: request.method,
    headers: proxyHeaders,
...
3. **Verify in GHOST ID dashboard**: Data should appear
...
3. If not cached, proxies to `app.ghost-id.io/api/script.js`
...
async function handleRequest(request) {
  ...
  try {
    const response = await proxyToGHOST ID(request, url);
...
1. Verify cache key includes important parts:
   ```javascript
   const cacheKey = new Request(ghost-idUrl, request);
   ```
