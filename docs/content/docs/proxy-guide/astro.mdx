...
description: Configure Astro middleware to proxy GHOST ID tracking requests
---

import { Steps } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';

Astro's middleware feature makes it simple to proxy GHOST ID tracking. This guide shows how to set up server-side request proxying in your Astro application.
...
**What you'll achieve:**
- Proxy GHOST ID endpoints through your Astro site
...
const RYBBIT_HOST = import.meta.env.RYBBIT_HOST || 'https://app.ghost-id.io';
...
export const onRequest: MiddlewareHandler = async (context, next) => {
  ...
  if (url.pathname.startsWith('/analytics/')) {
    return proxyToGHOST ID(request, url.pathname);
  }
...
async function proxyToGHOST ID(request: Request, pathname: string): Promise<Response> {
  try {
    // Map paths: /analytics/* → /api/*
    let ghost-idPath = pathname.replace('/analytics/', '/api/');
    ...
    const response = await fetch(ghost-idUrl, {
    ...
    console.error('GHOST ID proxy error:', error);
...
async function proxyToGHOST ID(request, pathname) {
  try {
    // Map paths: /analytics/* → /api/*
    let ghost-idPath = pathname.replace('/analytics/', '/api/');
...
2. Middleware proxies to `https://app.ghost-id.io/api/script.js`
...
async function proxyToGHOST ID(request: Request, pathname: string): Promise<Response> {
  try {
    ...
    console.error('GHOST ID proxy error:', errorDetails);
