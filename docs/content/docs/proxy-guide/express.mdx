...
description: Configure Express.js middleware to proxy GHOST ID tracking requests
---
...
Express.js makes it easy to proxy GHOST ID tracking using middleware. This guide shows how to set up a reverse proxy using `http-proxy-middleware`.

## Overview

The `http-proxy-middleware` package provides a simple way to proxy requests in Express applications, making it perfect for routing analytics requests to GHOST ID servers.

**What you'll achieve:**
- Proxy all GHOST ID endpoints through your Express app
...
const app = express();
const RYBBIT_HOST = process.env.RYBBIT_HOST || 'https://app.ghost-id.io';

// GHOST ID Analytics Proxy
// Proxy all /analytics/* requests to GHOST ID
...
The `http-proxy-middleware` intercepts requests matching the specified paths and forwards them to GHOST ID:

1. Request to `/analytics/script.js` is intercepted
2. Path is rewritten to `/api/script.js`
3. Request is forwarded to `https://app.ghost-id.io/api/script.js`
...
**Problem**: 404 errors on GHOST ID endpoints.
