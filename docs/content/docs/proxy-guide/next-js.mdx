...
description: Configure Next.js rewrites to proxy GHOST ID tracking and bypass ad blockers
---
...
Next.js provides a powerful [rewrites](https://nextjs.org/docs/app/api-reference/next-config-js/rewrites) feature that makes it easy to proxy GHOST ID tracking through your own domain. This guide works with both the App Router (Next.js 13+) and Pages Router.

## Overview

Next.js rewrites allow you to map incoming request paths to different destination URLs transparently. This is perfect for proxying analytics since the browser sees requests to your domain while Next.js forwards them to GHOST ID's servers behind the scenes.

**What you'll achieve:**
- Serve GHOST ID scripts from your domain (e.g., `/analytics/script.js`)
...
Create or update your `.env` or `.env.local` file with your GHOST ID instance URL:
...
```bash
# .env.local
NEXT_PUBLIC_RYBBIT_HOST=https://app.ghost-id.io
```
...
Replace `analytics.yourcompany.com` with your actual GHOST ID instance domain.
...
This configuration proxies all GHOST ID endpoints including session replay and Web Vitals:
...
const nextConfig = {
  async rewrites() {
    const ghost-idHost = process.env.NEXT_PUBLIC_RYBBIT_HOST;

    if (!ghost-idHost) {
      console.warn('NEXT_PUBLIC_RYBBIT_HOST is not set. GHOST ID proxy will not work.');
      return [];
    }
    ...
    return [
      // Tracking scripts
      {
        ...
        destination: `${ghost-idHost}/api/script.js`,
      },
      {
        source: '/analytics/replay.js',
        destination: `${ghost-idHost}/api/replay.js`,
      },
      {
        source: '/analytics/metrics.js',
        destination: `${ghost-idHost}/api/metrics.js`,
      },
      // Tracking endpoints
      {
        source: '/analytics/track',
        destination: `${ghost-idHost}/api/track`,
      },
      {
        source: '/analytics/identify',
        destination: `${ghost-idHost}/api/identify`,
      },
      {
        source: '/analytics/session-replay/record/:siteId',
        destination: `${ghost-idHost}/api/session-replay/record/:siteId`,
      },
      {
        source: '/analytics/site/tracking-config/:siteId',
        destination: `${ghost-idHost}/api/site/tracking-config/:siteId`,
      },
    ];
  },
};
...
<Callout type="info">
**Custom Path**: You can change `/analytics` to any path you prefer (e.g., `/tracking`, `/stats`, `/ghost-id`). Just ensure you use the same path consistently in both your rewrites and script tag.
</Callout>

### Update Your Tracking Script

Update your script tag to load from the proxied path instead of directly from GHOST ID:
...
2. **Verify the script loads**:
   - You should see a request to `/analytics/script.js` (not `app.ghost-id.io`)
...
3. **Verify tracking works**:
   ...
   - All requests should show your domain, not `app.ghost-id.io`
4. **Check the GHOST ID dashboard**:
...
The GHOST ID tracking script has a built-in auto-discovery mechanism. When it loads from:
...
This means all tracking requests automatically go to `/analytics/track`, `/analytics/identify`, etc., which Next.js then rewrites to forward to GHOST ID's servers.
...
The GHOST ID server sets appropriate cache headers, which Next.js respects:
...
4. Ensure your GHOST ID instance URL is correct in `.env.local`
