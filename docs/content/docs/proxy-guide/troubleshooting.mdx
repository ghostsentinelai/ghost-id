...
description: Common issues and solutions for GHOST ID proxy configurations
---

import { Callout } from 'fumadocs-ui/components/callout';
import { Steps } from 'fumadocs-ui/components/steps';

This guide covers common issues you might encounter when setting up a GHOST ID proxy and how to resolve them.
...
Verify GHOST ID backend is accessible:
...
- No tracking data appears in GHOST ID dashboard
...
Tracking endpoints need proper CORS headers. GHOST ID sets these automatically, but ensure your proxy preserves them:
...
// ✅ Correct - allow all origins (or use GHOST ID's headers)
...
proxy_pass https://app.ghost-id.io;
```
...
**Caddy:**
```txt
reverse_proxy https://app.ghost-id.io {
...
**Symptoms:**
- Changed settings in GHOST ID dashboard
...
sudo rm -rf /var/cache/nginx/ghost-id/*
```

```javascript
// Node.js/Express with node-cache
cache.del(`ghost-id_config_${siteId}`);
...
```nginx
# Nginx with caching
proxy_cache_path /var/cache/nginx/ghost-id levels=1:2 keys_zone=ghost-id:10m;

location = /analytics/script.js {
    proxy_cache ghost-id;
...
```bash
# Test latency to GHOST ID
curl -w "@curl-format.txt" -o /dev/null -s https://app.ghost-id.io/api/script.js
...
- Self-hosting GHOST ID closer to your users
- Using a CDN proxy
...
```bash
curl -v https://app.ghost-id.io/api/script.js
```
...
```bash
git add vercel.json
git commit -m "Add GHOST ID proxy config"
git push
```
...
5. **Verify directly** against `https://app.ghost-id.io/api/script.js`
