...
description: Configure Caddy reverse proxy to serve GHOST ID tracking with automatic HTTPS
---
...
Caddy is a powerful web server with automatic HTTPS that makes reverse proxying simple and secure. This guide shows how to configure Caddy to proxy GHOST ID tracking through your own domain.
...
yourdomain.com {
    ...
    handle /analytics/script.js {
        reverse_proxy https://app.ghost-id.io {
            header_up Host app.ghost-id.io
...
This proxies all `/analytics/*` requests to `https://app.ghost-id.io/api/*`.
...
3. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in GHOST ID dashboard

</Steps>

## How It Works

Caddy's `reverse_proxy` directive forwards requests to GHOST ID's servers:
...
4. Forwards to `https://app.ghost-id.io/api/script.js` with proper headers
5. Returns response to browser

The GHOST ID script auto-detects it's being served from your domain and sends all tracking to your domain's endpoints.
...
{$DOMAIN:yourdomain.com} {
    handle_path /analytics/* {
        reverse_proxy {$RYBBIT_HOST:https://app.ghost-id.io} {
...
export RYBBIT_HOST=https://app.ghost-id.io
caddy run
```
...
**Solution**:
1. Verify GHOST ID backend is accessible:
   ```bash
...
**Problem**: Requests go to wrong GHOST ID endpoints.
