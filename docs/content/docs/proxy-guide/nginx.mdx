...
description: Configure Nginx reverse proxy to serve GHOST ID tracking and bypass ad blockers
---
...
Nginx is a powerful web server and reverse proxy that makes it easy to proxy GHOST ID tracking through your own domain. This guide shows how to configure Nginx to forward tracking requests to GHOST ID servers while caching static scripts for better performance.

## Overview

Nginx's `proxy_pass` directive lets you forward requests from your domain to GHOST ID's servers. Combined with caching, this provides excellent performance and reliability for proxying analytics.

**What you'll achieve:**
- Proxy all GHOST ID endpoints through your domain
- Cache static scripts for better performance
- Forward necessary headers for accurate tracking
- Support all GHOST ID features (session replay, Web Vitals, custom events)

## Prerequisites

- Nginx installed (version 1.18 or later recommended)
- SSL/TLS certificate configured (Let's Encrypt recommended)
- Your GHOST ID instance URL:
  - **Cloud hosted**: `https://app.ghost-id.io`
  - **Self-hosted**: Your instance URL
- Your GHOST ID site ID (found in your dashboard)
...
server {
    ...
    # GHOST ID Analytics Proxy
    # Main tracking script (cached)
    location = /analytics/script.js {
        proxy_pass https://app.ghost-id.io/api/script.js;
        proxy_set_header Host app.ghost-id.io;
    ...
    location = /analytics/replay.js {
        proxy_pass https://app.ghost-id.io/api/replay.js;
        proxy_set_header Host app.ghost-id.io;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/metrics.js {
        proxy_pass https://app.ghost-id.io/api/metrics.js;
        proxy_set_header Host app.ghost-id.io;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/track {
        proxy_pass https://app.ghost-id.io/api/track;
        proxy_set_header Host app.ghost-id.io;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location = /analytics/identify {
        proxy_pass https://app.ghost-id.io/api/identify;
        proxy_set_header Host app.ghost-id.io;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/session-replay/record/(.*)$ {
        proxy_pass https://app.ghost-id.io/api/session-replay/record/$1;
        proxy_set_header Host app.ghost-id.io;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location ~ ^/analytics/site/tracking-config/(.*)$ {
        proxy_pass https://app.ghost-id.io/api/site/tracking-config/$1;
        proxy_set_header Host app.ghost-id.io;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
<Callout type="info">
Replace `app.ghost-id.io` with your self-hosted GHOST ID instance URL if you're not using the cloud version.
</Callout>
...
http {
    # Define cache zone for GHOST ID
    proxy_cache_path /var/cache/nginx/ghost-id
        levels=1:2
        keys_zone=ghost-id_cache:10m
        max_size=100m
        inactive=60m
        use_temp_path=off;
...
```bash
sudo mkdir -p /var/cache/nginx/ghost-id
sudo chown www-data:www-data /var/cache/nginx/ghost-id
```
...
Replace `YOUR_SITE_ID` with your actual site ID from the GHOST ID dashboard.
...
4. **Open your website** in a browser with Developer Tools and verify:
   ...
   - Data appears in your GHOST ID dashboard after 1-2 minutes

</Steps>

## How It Works

Nginx's `proxy_pass` directive transparently forwards requests to GHOST ID's servers:

1. Browser requests `https://yourdomain.com/analytics/script.js`
2. Nginx matches the location block and forwards to `https://app.ghost-id.io/api/script.js`
...
The GHOST ID script auto-detects it's being served from your domain and sends all tracking data to your domain's `/analytics/*` endpoints.
...
location ~ ^/analytics/(script|replay|metrics)\.js$ {
    proxy_pass https://app.ghost-id.io/api/$1.js;
    # ... proxy headers
    proxy_cache_valid 200 1h;
...
location ~ ^/analytics/(track|identify)$ {
    proxy_pass https://app.ghost-id.io/api/$1;
    # ... proxy headers
...
Keep connections to GHOST ID backend alive for better performance:

```nginx
upstream ghost-id_backend {
    server app.ghost-id.io:443;
    keepalive 32;
}

location /analytics/ {
    proxy_pass https://ghost-id_backend/api/;
...
```nginx
location ~ ^/analytics/session-replay/record/ {
    proxy_pass https://app.ghost-id.io/api/session-replay/record/;
    # ... proxy headers
...
**Solution**:
1. Check GHOST ID backend is accessible:
   ```bash
   curl -I https://app.ghost-id.io/api/script.js
   ```
...
### Incorrect geolocation in GHOST ID
...
**Problem**: Logs show SSL verification errors when connecting to GHOST ID.

**Solution**:
Add `proxy_ssl_server_name on` to enable SNI:
```nginx
location /analytics/ {
    proxy_pass https://app.ghost-id.io/api/;
...
server {
    ...
    # Proxy all requests to GHOST ID
    location / {
        proxy_pass https://app.ghost-id.io/api/;
        proxy_set_header Host app.ghost-id.io;
        proxy_set_header X-Real-IP $remote_addr;
...
### Multiple GHOST ID Instances

If you need to proxy multiple GHOST ID instances:

```nginx
location /analytics-prod/ {
    proxy_pass https://prod.ghost-id.io/api/;
    # ... proxy config
}

location /analytics-staging/ {
    proxy_pass https://staging.ghost-id.io/api/;
    # ... proxy config
}
```
...
- [Nginx Custom Configuration Guide](/docs/self-hosting-guides/custom-nginx) - Self-hosting GHOST ID with Nginx
