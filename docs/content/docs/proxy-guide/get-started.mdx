...
description: Framework-agnostic guide to proxy GHOST ID tracking through your own domain
---
...
This guide covers the core concepts for setting up a GHOST ID proxy with any reverse proxy or server framework. If your specific framework has a dedicated guide, we recommend following that instead for optimized instructions.
...
- **Your GHOST ID instance URL**:
  - Cloud: `https://app.ghost-id.io`
  - Self-hosted: Your GHOST ID instance URL (e.g., `https://analytics.yourcompany.com`)
- **Your GHOST ID site ID** (found in your GHOST ID dashboard)

## Understanding the Endpoints

GHOST ID uses several endpoints for tracking. You can choose between a minimal setup or full proxy:
...
This setup supports all GHOST ID features including session replay, Web Vitals, and custom events.
...
Set up your reverse proxy to forward requests from your chosen path (e.g., `/analytics/*`) to the GHOST ID server.

**Generic proxy configuration pattern:**

```
Your domain path          →  GHOST ID backend
-----------------            ---------------
/analytics/script.js      →  https://app.ghost-id.io/api/script.js
/analytics/track          →  https://app.ghost-id.io/api/track
/analytics/replay.js      →  https://app.ghost-id.io/api/replay.js
... and so on
```
...
Implement caching to reduce load on GHOST ID servers and improve performance:
...
Change your script tag to load from your proxied domain instead of directly from GHOST ID:

**Before (direct loading):**
```html
<script src="https://app.ghost-id.io/api/script.js" async data-site-id="YOUR_SITE_ID"></script>
```
...
<Callout type="info">
**How Auto-Discovery Works**: The GHOST ID script automatically extracts the analytics host from its own `src` attribute by splitting on `/script.js`. This means when it loads from `https://yourdomain.com/analytics/script.js`, it automatically sends all tracking data to `https://yourdomain.com/analytics/*` endpoints.
</Callout>
...
5. **Check your GHOST ID dashboard**:
...
**CORS headers** (should be preserved from GHOST ID):
```
...
For accurate tracking, ensure these headers are forwarded to the GHOST ID backend:
...
<Callout type="warn">
**IP Forwarding**: GHOST ID uses IP addresses for geolocation (country/city) and creating anonymous user identifiers (hashed IP + User Agent). If you don't forward the `X-Forwarded-For` or `X-Real-IP` header, all traffic will appear to come from your proxy server's IP.
</Callout>
...
Configure your proxy to maintain persistent connections to GHOST ID's backend to reduce latency on repeated requests.
...
```
https://analytics.yourdomain.com/script.js  →  https://app.ghost-id.io/api/script.js
https://analytics.yourdomain.com/track      →  https://app.ghost-id.io/api/track
```
...
```
https://yourdomain.com/analytics/script.js  →  https://app.ghost-id.io/api/script.js
https://yourdomain.com/analytics/track      →  https://app.ghost-id.io/api/track
```
...
- Whitelist your GHOST ID backend IP if possible
...
2. **CORS errors**: Ensure CORS headers are preserved from GHOST ID backend
