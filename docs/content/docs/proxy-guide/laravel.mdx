...
description: Configure Laravel routes and middleware to proxy GHOST ID tracking
---
...
Laravel makes it straightforward to proxy GHOST ID tracking using routes and the HTTP client. This guide shows how to set up proxy endpoints in your Laravel application.

## Overview

Laravel's HTTP client (built on Guzzle) provides an elegant way to proxy requests to GHOST ID servers while maintaining full control over headers, caching, and error handling.

**What you'll achieve:**
- Proxy all GHOST ID endpoints through your Laravel app
- Forward necessary headers for accurate tracking
- Optional caching with Laravel Cache
- Support all GHOST ID features
...
Add your GHOST ID host to `.env`:

```bash
# .env
RYBBIT_HOST=https://app.ghost-id.io
...
{
    private string $ghost-idHost;

    public function __construct()
    {
        $this->ghost-idHost = config('services.ghost-id.host', 'https://app.ghost-id.io');
    }
    ...
    public function proxyScript(Request $request, string $script)
    {
        ...
        $cacheKey = "ghost-id_script_{$script}";
    ...
    public function proxySiteConfig(Request $request, string $siteId)
    {
        ...
        $cacheKey = "ghost-id_config_{$siteId}";
    ...
    private function forwardRequest(string $path, string $method, Request $request)
    {
        ...
        try {
            ...
            \Log::error('GHOST ID proxy error', [
                'url' => $url,
                'error' => $e->getMessage(),
            ]);
...
Add GHOST ID configuration to `config/services.php`:
...
return [
    ...
    'ghost-id' => [
        'host' => env('RYBBIT_HOST', 'https://app.ghost-id.io'),
    ],
];
```
...
4. **Verify in GHOST ID dashboard**: Data should appear

</Steps>

## How It Works

Laravel routes intercept requests to `/analytics/*` and forward them to GHOST ID:

1. Request to `/analytics/script.js` hits Laravel route
2. Controller forwards to `https://app.ghost-id.io/api/script.js`
...
public function proxySessionReplay(Request $request, string $siteId)
{
    ...
    ForwardAnalyticsData::dispatch(
        "{$this->ghost-idHost}/api/session-replay/record/{$siteId}",
        $request->all(),
        [
...
**Problem**: Old script cached after GHOST ID update.

**Solution**:
Clear specific cache key:
```bash
php artisan cache:forget ghost-id_script_script.js
```
...
Cache::tags(['ghost-id', 'scripts'])->remember($cacheKey, 3600, function () {
    // ... fetch script
});

// Clear all GHOST ID cache
Cache::tags(['ghost-id'])->flush();
```
...
Http::pool(fn (Pool $pool) => [
    $pool->get("{$this->ghost-idHost}/api/script.js"),
    $pool->get("{$this->ghost-idHost}/api/replay.js"),
]);
```
