---
title: Track Events
description: How to track custom events with GHOST ID
---

The script exposes a global object `window.ghost-id` with functions for manual tracking of events, as well as the ability to track events using data attributes on HTML elements.
...
<button
  data-ghost-id-event="purchase_click"
  data-ghost-id-prop-product="premium"
  data-ghost-id-prop-price="49.99"
>
  Buy Now
</button>
```
...
### `window.ghost-id.event(eventName, properties)`
...
window.ghost-id.event("Signup Button Clicked");

// Track a custom event with properties
window.ghost-id.event("Item Added To Cart", {
  itemId: "PROD123",
  price: 49.99,
  category: "Clothing"
});
...
Put this as `ghost-id.d.ts` anywhere in your project to avoid having to do `(window as any).ghost-id.event()` everywhere.
