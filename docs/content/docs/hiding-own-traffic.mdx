...
There are several ways to disable GHOST ID tracking to hide your own visits from analytics.
...
<Tabs items={['IP Exclusion', 'Browser Extensions', 'localStorage Method']}>
  <Tab>
    **IP Exclusion (Recommended)**: Block traffic from specific IP addresses or ranges at the server level for complete accuracy.
    ...
    1. **Navigate to Site Settings**: Go to your site's settings page in the GHOST ID dashboard
    ...
    GHOST ID supports multiple IP exclusion formats:
  ...
  <Tab>
    **Browser Extensions**: Easy client-side solution using community-maintained browser extensions.
    ...
    - **[Firefox Extension](https://addons.mozilla.org/en-US/firefox/addon/ghost-id-opt-out/)**: Install from Firefox Add-ons

    These extensions automatically disable GHOST ID tracking on all websites where you have GHOST ID installed, without requiring any manual configuration.

    <Callout type="info">
    **Self-hosted users**: These extensions require GHOST ID version 1.1.0 or higher. If you're running an older version, please update your installation or use the localStorage method.
    </Callout>
  ...
  <Tab>
    **localStorage Method**: Manual configuration using your browser's developer console for more granular control.
    ...
    ```js
    localStorage.setItem('disable-ghost-id', 'true');
    ```
    ...
    ```js
    localStorage.removeItem('disable-ghost-id');
    ```
...
- **Browser extensions** offer convenience by automatically working across all your GHOST ID-enabled websites  
