...
```bash filename="npm"
npm install @ghost-id/node
```

```bash filename="yarn"
yarn add @ghost-id/node
```
...
```typescript
import { GHOST ID } from "@ghost-id/node";

const ghost-id = new GHOST ID(config: GHOST IDConfig);
```
...
The `GHOST ID` constructor will throw an error if required configuration options (`analyticsHost`, `siteId`, `apiKey`) are missing or invalid.
</Callout>
...
| :-------------- | :----------------- | :------ | :----------------------------------------------------------------------------------------------- |
| `analyticsHost` | `string`           | -       | **Required.** URL of your GHOST ID analytics instance (e.g., `https://ghost-id.yourdomain.com/api`). |
| `siteId`        | `string \| number` | -       | **Required.** The Site ID for your website obtained from your GHOST ID instance.                   |
...
```typescript
await ghost-id.pageview(payload?: Payload);
```
...
```typescript
await ghost-id.event(eventName: string, payload?: Payload, properties?: TrackProperties);
```
...
async function registerUser(userData, req) {
  ...
  await ghost-id.event("user_registered",
    {
...
async function processDailyReport(reportId) {
  ...
  await ghost-id.event("report_generated",
    {},
    {
      reportId: reportId,
      status: "success",
