...
GHOST ID can automatically capture common user interactions without requiring any code changes. These features can be enabled in your [Site Settings](/docs/site-settings) under the "Auto Capture" section.
...
When enabled, GHOST ID automatically tracks clicks on links that navigate to external domains.
...
You can add custom properties to auto-tracked button clicks using `data-ghost-id-prop-*` attributes:

```html
<button
  data-ghost-id-prop-category="pricing"
  data-ghost-id-prop-variant="blue"
>
  Buy Now
</button>
```
...
If you want to track a button with a custom event instead of the automatic `button_click`, add the `data-ghost-id-event` attribute:

```html
<!-- This button will fire a custom "signup" event, NOT a button_click -->
<button data-ghost-id-event="signup" data-ghost-id-prop-plan="pro">
  Sign Up
</button>
```
...
You can add custom properties using `data-ghost-id-prop-*` on the form element:

```html
<form
  id="signup"
  data-ghost-id-prop-form-type="registration"
  data-ghost-id-prop-variant="a"
>
  ...
</form>
```
...
You can add custom properties using `data-ghost-id-prop-*` on the input element:

```html
<input
  name="email"
  data-ghost-id-prop-field-category="contact"
/>

<select
  name="country"
  data-ghost-id-prop-section="demographics"
>
  ...
</select>
```
